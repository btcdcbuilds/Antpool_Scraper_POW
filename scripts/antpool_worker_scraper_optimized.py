#!/usr/bin/env python3
"""
Antpool Worker Scraper - Performance Optimized Version

This script scrapes worker statistics from Antpool with improved error handling,
retry logic, and performance optimizations including reduced wait times and parallel processing.
"""
...
async def process_single_client(access_key: str, user_id: str, coin_type: str, output_dir: str, debug: bool = False) -> Dict[str, Any]:
    """Process a single client and scrape their worker data."""
    browser = None
    page = None
    ...
    try:
        ...
        # Setup browser
        browser, page = await setup_browser(headless=True)
        ...
        output_file = os.path.join(output_dir, f"{timestamp_str}_Antpool_{coin_type}_workers.json")
...
async def process_accounts_batch(accounts_batch, output_dir):
    """Process a batch of accounts concurrently."""
    tasks = []
    for account in accounts_batch:
        task = asyncio.create_task(process_single_account(account, output_dir))
        tasks.append(task)
    ...
    for i, result in enumerate(results):
        account = accounts_batch[i]
        if isinstance(result, Exception):
            logger.error(f"Error processing account {account['user_id']}: {str(result)}")
...
async def process_single_account(account, output_dir):
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Antpool Worker Scraper - Optimized Version')
    parser.add_argument('--batch-size', type=int, default=5, help='Number of accounts to process concurrently (default: 5)')
    ...
    logger.info(f"Processing {len(accounts)} accounts in {len(account_batches)} batches of {batch_size}")
    
    all_results = []
    for batch_idx, batch in enumerate(account_batches):
        logger.info(f"Processing batch {batch_idx + 1}/{len(account_batches)} ({len(batch)} accounts)")
        batch_results = await process_accounts_batch(batch, output_dir)
        all_results.extend(batch_results)
    
    # Summarize results
    successful_accounts = sum(1 for r in all_results if r["result"]["success"])
    failed_accounts = len(all_results) - successful_accounts
